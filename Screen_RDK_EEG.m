    ...
    while trial <= size(info.matrix,1)
        ...
        if info.matrix(trial,5) ~= 0
            ...
            if info.matrix(trial,1) == 1
                Screen('DrawDots', win, [0 0], info.fp_size_pix_white*3, trl.cue_blue, [info.scr_xcenter info.scr_ycenter-110], []);
            else
                Screen('DrawDots', win, [0 0], info.fp_size_pix_white*3, trl.cue_pink, [info.scr_xcenter info.scr_ycenter-110], []);
            end

            % beginning of a new color block
            if info.matrix(trial,5) == 2
                if trial ~= 1
                    if info.matrix(trial,1) == 1
                        txt6 = 'ATENÇÃO À MUDANÇA DE COR!';
                        DrawFormattedText(win, txt6, 'center', info.scr_ycenter -160, trl.cue_blue);
                    else
                        txt6 = 'ATENÇÃO À MUDANÇA DE COR!';
                        DrawFormattedText(win, txt6, 'center', info.scr_ycenter -160, trl.cue_pink);
                    end
                else
                    txt6 = 'ATENÇÃO À COR!';
      ...
      %  Eyelink('ImageTransfer', '/home/kaneda/Documents/GitHub/PSA_RDK/Images/trl_on.bmp', 0, 0, 0, 0, 0, 0);
        ...
        for frame = 1:info.matrix(trial,8)
            ...
            % time limit (700 ms)
            if frame >= info.matrix(trial,7) && SRT2 == 2

                if info.matrix(trial,1) == 1 % BLUE

                    if info.matrix(trial,2) ==1 % BLUE LEFT
                        Screen('BlendFunction',win,GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
                        Screen('FillArc', win, trl.cue_pink, info.cue_position, 0, 180); % Right
                        Screen('FillArc', win, trl.cue_blue, info.cue_position, 180, 180); % Left
                    else                        % BLUE RIGHT
                        Screen('BlendFunction',win,GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
                        Screen('FillArc', win, trl.cue_blue, info.cue_position, 0, 180); % Right
                        Screen('FillArc', win, trl.cue_pink, info.cue_position, 180, 180); % Left
                    end
                else                        % PINK
                    if info.matrix(trial,2) ==1 % PINK LEFT
                        Screen('BlendFunction',win,GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
                        Screen('FillArc', win, trl.cue_blue, info.cue_position, 0, 180); % Right
                        Screen('FillArc', win, trl.cue_pink, info.cue_position, 180, 180); % Left
                    else                        % PINK RIGHT
            ...
            if frame >= info.matrix(trial,7)
                ...
                if info.matrix(trial,2) == 1
                    if inFixWindow(x_gaze,y_gaze,fix_win_left)
                        saccade_left = 1; % saccade executed to the correct side
                    end
                else
                    if inFixWindow(x_gaze,y_gaze,fix_win_right)
                        saccade_right = 1; % saccade executed to the correct side
                    end
                end

            elseif frame < info.matrix(trial,7) % checks fixation violation before cue onset
                if ~inFixWindow(x_gaze,y_gaze,fix_win_center)
                    if FIX == 2
                        FIX = 3;
                    end
        ...
        if FIX == 3
            ...
            txt1 = 'Mantenha a fixação até o aparecimento da pista!';
        ...
        elseif info.matrix(trial,2) == 1 && saccade_left == 2 || ...
            ...
            if info.matrix(trial,1) == 1
                txt2 = 'Olhe para o objeto indicado pela cor Azul!';
                DrawFormattedText(win, txt2, 'center', info.scr_ycenter, trl.cue_blue);
            else
                txt2 = 'Olhe para o objeto indicado pela cor Rosa!';
        ...
        if info.matrix(trial,10) == 0
            ...
            if abort_dir_report == 1
                ...
                repeat_trl_blk = repeat_trl_blk + 1;
                repeat_trials(repeat_trl_blk,1) = trial;
        ...
        % the end of a short block with resting time (2)
        if  info.matrix(trial,6) ~= 0

            if trial == size(info.matrix,1)
                txt = 'Voce completou todos os blocos da sessão. \n\n Parabéns!';
                DrawFormattedText(win, txt, 'center', info.scr_ycenter, info.white_idx);

            elseif info.matrix(trial,6) == 1
